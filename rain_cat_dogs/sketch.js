...
let cloud_img
let animal_imgs = []
let animal_assets = ['assets/animals/red_dog1.png', 
...
let animals = []
let animal_min_width = 10
let animal_max_width = 100

let bclouds = []
let bcloud_min_width = 50
let bcloud_max_width = 100

let fclouds = []
let fcloud_min_width = 100
let fcloud_max_width = 200
...
function preload() {
	...
	for (let animal_asset of animal_assets) {
...
function setup() {
	...
	for (let i = 0; i < 30; i++) {
		let c = new Cloud(random(0.1, 0.5), bcloud_min_width, bcloud_max_width)
		bclouds.push(c)
	}

	for (let i = 0; i < 4; i++) {
		let c = new Cloud(random(0.3, 1.5), fcloud_min_width, fcloud_max_width)
		fclouds.push(c)
	}
}
...
function resize_height(img, resize_width) {
	let im_w = img.width
	let im_h = img.height
	let resize_h = im_h / im_w * resize_width
