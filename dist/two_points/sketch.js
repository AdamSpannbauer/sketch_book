...
function draw() {
	...
	let y = createVector(mouseX, mouseY)
	let dist_calc = sel.value()
...
function mid_point_text(p1, p2, t) {
	let mp = createVector((p2.x - p1.x) / 2, (p2.y - p1.y) / 2)
...
function euclidean_distance(p1, p2, name, text_y) {
	let dist = sqrt((p1.x - p2.x)**2 + (p1.y - p2.y)**2)
...
function manhattan_distance(p1, p2, name, text_y) {
	let dist_x = abs(p1.x - p2.x)
	let dist_y = abs(p1.y - p2.y)
	let dist = dist_x + dist_y

	if (dist_calc == 'Manhattan') {
		let l1p1 = createVector(p1.x, p1.y)
		let l1p2 = createVector(p2.x, p1.y)
		let l2p1 = createVector(p2.x, p1.y)
		let l2p2 = createVector(p2.x, p2.y)
...
function chebyshev_distance(p1, p2, name, text_y) {
	...
	if (dist_calc == 'Chebyshev') {
		let lp1 = createVector(p1.x, p1.y)
		let lp2
		if (dist == dist_x) {
